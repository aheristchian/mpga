...
export function calculateVotingThreshold(
  aliveCount: number,
  rounding: 'ceil' | 'floor' | 'half' | string = 'ceil'
): number {
 ...
 */
export function calculateMaxVotesPerCandidate(aliveCount: number): number {
 ...
 */
export function clampVotes(currentVotes: number, delta: number, aliveCount: number): number {
  const max = calculateMaxVotesPerCandidate(aliveCount);
  return Math.min(max, Math.max(0, (currentVotes || 0) + delta));
}

export interface TogglePreVoteResult {
  changed: boolean;
  added: boolean;
  voterChoices: string[];
}
 ...
 */
export function togglePreVote(
  voterName: string,
  candidateName: string,
  voterMap: Record<string, Set<string> | string[]>,
  ...
  if (!voterMap[normalizedVoter]) {
    voterMap[normalizedVoter] = new Set<string>();
  } else if (Array.isArray(voterMap[normalizedVoter])) {
    voterMap[normalizedVoter] = new Set<string>(voterMap[normalizedVoter] as string[]);
  }

  const voterSet = voterMap[normalizedVoter] as Set<string>;
...
export interface CastFinalVoteResult {
  changed: boolean;
...
export function castFinalVote(
  voterName: string,
  defenderName: string,
  voterMap: Record<string, string | null>,
  counts: Record<string, number>,
  aliveCount: number
): CastFinalVoteResult {
  if (
